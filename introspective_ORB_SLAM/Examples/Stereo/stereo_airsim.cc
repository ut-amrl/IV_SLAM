...
DEFINE_string(img_qual_path, "", "Path to quality images for feature "
             ...
             "image.");
DEFINE_string(introspection_model_path, "", "Path to the trained model " 
...
DEFINE_bool(introspection_func_enabled, false, "Enables the introspection function.");
...
int main(int argc, char **argv)
{
    ...
    string usage("This program runs stereo ORB-SLAM on airsim format "
    ...
    if (FLAGS_help) {
      gflags::ShowUsageWithFlagsRestrict(argv[0], "stereo_airsim");
      return 0;
    }
    CheckCommandLineArgs(argv);
    
    if(FLAGS_load_gt_depth_imgs && FLAGS_introspection_func_enabled) {
      LOG(FATAL) << "Ground truth depth images cannot be passed through " 
                 << "when the introspection function is enabled." << endl;
    }
  ...
  //         cv::waitKey(0);
        ...
        } else if (FLAGS_introspection_func_enabled) {
          if (FLAGS_load_img_qual_heatmaps) {
            ...
            cv::resize(imLeft_RGB,
                       imLeft_RGB, cv::Size(512, 512));
            ...
            at::Tensor cost_img = introspection_func.forward(inputs).toTensor();
            cost_img = (cost_img * 255.0).to(torch::kByte);
            cost_img = cost_img.to(torch::kCPU);

            auto cost_img_cv = ToCvImage(cost_img);
            resize(cost_img_cv, cost_img_cv, 
                  cv::Size(imLeft.cols, imLeft.rows));
            // ShowImage(cost_img_cv, "Predicted cost image");

            depth_im = cost_img_cv;
          }
        }
        ...
        bool pose_cov_available = kApplyNoiseToCamPose && kPassPoseCov;
        ...
        if (FLAGS_ivslam_enabled) {
          SLAM.TrackStereo(imLeft,
                           ...
                           tframe, 
                           cam_poses_gt[ni],
                           cam_poses_gt_cov[ni].cast<double>(),
                           pose_cov_available,
                           img_name, 
...
void LoadImagesWithGT(const string &strPathToSequence,
    ...
    for(int i=0; i<nTimes; i++)
    {
        ...
        vstrImageRight[i] = strPrefixRight + ss_10sig.str() + ".png";
