...
System::System(const string &strVocFile,
  ...
  if (bCreateIntrospectionDataset) {
    ...
    strModifiedDataPath =
        strModifiedDataPath.substr(0, pos0) + "Comprehensive/" + session_suffix;
    cout << "Comprehensive Dataset Path: " << endl;
    cout << strModifiedDataPath << endl;
    mpTracker->mvOutputIntrospectionDatasetFullPath = strModifiedDataPath;
  }
...
cv::Mat System::TrackMonocular(const cv::Mat &im,
  ...
  cv::Mat Tcw = mpTracker->GrabImageMonocular(
      im, timestamp, cam_pose_gt, img_name, gtDepthAvailable, depthmap);
