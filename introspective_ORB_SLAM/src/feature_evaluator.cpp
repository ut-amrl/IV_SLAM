...
void FeatureEvaluator::LoadImagePair(cv::Mat img_prev, 
                                     cv::Mat img_curr) {
...
void FeatureEvaluator::UpdateCameraCalibration(const ORB_SLAM2::Frame& frame) {
  projection_mat_cam0_ <<
    frame.fx, 0,        frame.cx, 0,
    0,        frame.fy, frame.cy, 0, 
    0,        0,        1.0,      0;
    
  projection_mat_cam1_ <<
    frame.fx, 0,        frame.cx, -frame.mb,
    0,        frame.fy, frame.cy, 0, 
    0,        0,        1.0,      0;
    
  camera_mat_ = (Mat_<double>(3,3) << frame.fx,   0,        frame.cx,
                                        0,        frame.fy, frame.cy,
                                        0,        0,        1.0);
...
cv::Mat FeatureEvaluator::GetAnnotatedImg() {
  if (dataset_ == kEuroc) {
    ...
    cv::Mat scaled_img;
    cv::resize(img_matching_annotation_, 
          scaled_img, cv::Size(), 0.7, 0.7);
    return scaled_img;
  } else {
...
cv::Mat FeatureEvaluator::GetBadMatchAnnotatedImg() {
  if (dataset_ == kEuroc) {
    ...
    cv::Mat scaled_img;
    cv::resize(img_bad_matching_annotation_, 
          scaled_img, cv::Size(), 0.7, 0.7);
    return scaled_img;
  } else {
    LOG (FATAL) << "Unknown dataset!";
...
cv::Mat FeatureEvaluator::GetUndistortedImg() {
  return img_prev_;
}
...
//     Scalar green = Scalar(0, 255, 0);
    ...
    if (auto_color_scaling) {
      ...
      double err_min = *min_element(std::begin(err_vals), 
                                    std::end(err_vals));
      double err_max = *max_element(std::begin(err_vals), 
                                    std::end(err_vals));
...
cv::Mat FeatureEvaluator::ColorKeypoints(
      cv::Mat& img,
...
//     Scalar green = Scalar(0, 255, 0);
    ...
    if (auto_color_scaling) {
      // Find the max and min epipolar error values
      double err_min = *min_element(std::begin(scalars), 
                                    std::end(scalars));
      double err_max = *max_element(std::begin(scalars), 
                                    std::end(scalars));
...
std::vector<float> FeatureEvaluator::GetErrValues() {
  return err_vals_select_;
}
...
void FeatureEvaluator::GenerateImageQualityHeatmap() {
  ...
  int bin_num_x = std::floor((double(img_curr_.cols) - kBinSizeX_)  
                             / kBinStride_) + 1;
  int bin_num_y = std::floor((double(img_curr_.rows) - kBinSizeY_) 
                             / kBinStride_) + 1;
...
//   bin_score = 1.0 * log(bin_mean_val) + 1.0 * (1/(1 + freq.cast<double>()));
  ...
  cv::Mat bad_region_heatmap_low_res = GenerateErrHeatmap(bin_num_y,
                                           bin_num_x, 
                                           bin_score_vec);
...
void FeatureEvaluator::GenerateUnsupImageQualityHeatmap(ORB_SLAM2::Frame& frame,
  ...
  for(size_t i =0; i < idx_interest.size(); i++) {
    err_vals_vec[i] = (2 / (1 + frame.mvKeyQualScoreTrain[idx_interest[i]])) 
                      - 1;
...
//   bin_score = 1.0 * log(bin_mean_val) + 1.0 * (1/(1 + freq.cast<double>()));
  ...
  addWeighted(left_img_color, 0.5, heatmap_colored, 0.5, 0.0, 
                                  overlaid_heatmap);
...
void FeatureEvaluator::GenerateUnsupImageQualityHeatmapGP(
  ...
  MatrixXf kmat = Kmatrix (point_loc);
  Eigen::Map<VectorXf> err_vals(err_vals_vec.data(),
                                err_vals_vec.size());  
  ...
  cv::Mat bad_region_var_heatmap_low_res = GenerateErrHeatmap(bin_num_y,
                                           bin_num_x, 
                                           grid_qual_var_vec,
                                           kGPVarMaxThresh,
                                           0.0);
...
bool FeatureEvaluator::DrawEpipolarErrVec(bool was_recently_reset) {
  ...
  for(size_t i = 0; i < keypts2_select_.size(); i++) {
    ...
    arrowedLine(img_epipolar_err_vec_, keypts2_select_[i].pt, 
                point_end, blue, 2, 8, 0);
...
//        abs_err_values.push_back(sigma_pts_err_[i][j].norm()); 
      ...
      float abs_err_std = sqrt(1.0 / 
                            static_cast<float>(sigma_pts_err_[i].size()) 
                          * squared_sum);      
    }
    
    if (draw_covariance && !epi_err_vec_particles_cov_.empty()) {
      Eigen::SelfAdjointEigenSolver<Eigen::Matrix2f> 
                          solver(epi_err_vec_particles_cov_[i]);
...
//         string txt = keypt_life + "," + keypt_ref_offset;
        ...
        cv::Size txt_size = getTextSize(txt, 
                               FONT_HERSHEY_SIMPLEX, font_scale, 1, &baseline);
        ...
        putText(img_epipolar_err_vec_, 
                txt, 
                cursor, 
                 FONT_HERSHEY_SIMPLEX, font_scale, blue, 1);
      }
    }
    ...
    if (draw_gt_reprojection) {
      if(!reproj2_select_gt_.empty()) {
        ...
        arrowedLine(img_epipolar_err_vec_, 
                    ...
                    blue, 2, 8, 0);
        cv::drawMarker (img_epipolar_err_vec_, reproj2_select_gt_[i], brown, 
                        MARKER_CROSS, 10, 2, 8);
      }
    }
  }
  ...
  if (draw_bad_match_percent) {
    ...
    cv::Size txt_size = getTextSize(txt, 
                            FONT_HERSHEY_SIMPLEX, font_scale, 3, &baseline);
    ...
    putText(img_epipolar_err_vec_, 
            txt, 
            cursor, 
              FONT_HERSHEY_SIMPLEX, font_scale, txt_color, 3);
  }
  ...
  if (draw_reset_notification && was_recently_reset) {
    ...
    cv::putText(img_epipolar_err_vec_,txt,
...
void FeatureEvaluator::SaveImagesToFile(std::string target_path,
  ...
  string feature_qual_path = target_path + "/feature_qual/" 
        + img_name_truncated + ".jpg";
  string feature_matching_path = target_path + "/feature_matching/" 
        + img_name_truncated + ".jpg";
  string bad_matched_features_path = target_path +"/bad_matched_features/" 
        + img_name_truncated + ".jpg";
  string bad_region_heatmap_path = target_path +"/bad_region_heatmap/" 
        + img_name_truncated + ".jpg";
  string bad_region_heatmap_vis_path = target_path +"/bad_region_heatmap_vis/" 
        + img_name_truncated + ".jpg";
  string bad_region_heatmap_masked_vis_path = target_path 
        +"/bad_region_heatmap_masked_vis/" + img_name_truncated + ".jpg";
  string reproj_err_vec_path = target_path +"/reprojection_err_vec/" 
        + img_name_truncated + ".jpg";
  string epipolar_err_vec_path = target_path +"/epipolar_err_vec/" 
        + img_name_truncated + ".jpg";
  string err_norm_factor_path = target_path +"/err_norm_factor/" 
        + img_name_truncated + ".jpg";
...
//   cv::imwrite(bad_region_heatmap_path, bad_region_heatmap_);
  ...
  if(kSaveColoredHeatmaps || kSaveColoredMaskedHeatmaps) {
      ...
      if(rectification_map_available_) {
        ...
        if(kDrawSelectedForTrainingFlag) {
          circle(heatmap_overlaid_unrect, 
                 Point(480, 20), 15 , flag_color, -1, 8, 0);
        }
    ...
    if(kSaveColoredMaskedHeatmaps) {
      ...
      if(kDrawSelectedForTrainingFlag) {
        circle(heatmap_overlaid_masked, Point(480, 20), 15 , flag_color,-1,8,0);
      }
...
bool FeatureEvaluator::GetGTReprojection(
  ...
  float x = depth * keypt.x / ref_frame.fx - 
            depth * ref_frame.cx / ref_frame.fx;
  float y = depth * keypt.y / ref_frame.fy - 
            depth * ref_frame.cy / ref_frame.fy;
  ...
  cv::Mat transform = CalculateRelativeTransform(curr_frame.mTwc_gt,
                                                 ref_frame.mTwc_gt);
  ...
  if (pt3d_curr.at<float>(2) < 0) {
    CHECK_EQ(curr_frame.mvKeysUn.size(),
            curr_frame.mvKeysGTDepth.size());
  
    cv::Point2f keypt(curr_frame.mvKeysUn[keypt_idx_in_curr].pt);
    float depth = curr_frame.mvKeysGTDepth[keypt_idx_in_curr];
    float x = depth * keypt.x / curr_frame.fx - 
              depth * curr_frame.cx / curr_frame.fx;
    float y = depth * keypt.y / curr_frame.fy - 
              depth * curr_frame.cy / curr_frame.fy;
...
bool FeatureEvaluator::GetGTReprojection(
  ...
  CHECK_EQ(ref_keyframe.mvKeysUn.size(),
           ref_keyframe.mvKeysGTDepth.size());
...
double FeatureEvaluator::CalculateNormalizedEpipolarErrorAnalytical(
  ...
  bool cam_pose_cov_available = ref_frame_cov_available &&
                                curr_frame.mbPoseUncertaintyAvailable;
  ...
  if (cam_pose_cov_available) {
    tf_prev_to_curr = CalculateRelativeTransform(
                        curr_frame.mTwc_gt,
                        ref_frame_Twc_gt,
...
//     cout << sigma_tf_prev_to_curr << endl;
    ...
    bool rel_pose_unc_retreived= GetRelativePoseUncertainty(ref_frame_name,
                                    curr_frame.mstrLeftImgName,
                                    &sigma_tf_prev_to_curr);
  ...
  if (cam_pose_cov_available || rel_cam_pose_uncertainty_available_) {
    ...
    sigma_w << kAngualrVariance, 0 ,   0, 
                  0, kAngualrVariance, 0,
                  0, 0, kAngualrVariance;
                  
    sigma_t << kTranslationalVariance, 0, 0,
              0, kTranslationalVariance, 0,
              0, 0, kTranslationalVariance;                      
  }
...
// transformation between the two frames
  ...
  Eigen::Vector3d x(static_cast<double>(keypoint2.pt.x), 
                    ...
                    1.0);
  GetEpipolarErrorJacobians(curr_frame,
                            R_prev_to_curr,
                            trans_prev_to_curr,
                            x_ref,
                            x,
...
//   cout << "var: " << var << ", keypt_sigma: " << keypt_sigma2 << endl;
  ...
  *err_variance = var;
  ...
  if (kNormalizationFactor > 0 && 
      !cam_pose_cov_available && 
  ...
  normalization_factor = (normalization_factor < kMinEpipolarErrSensitivity_)?
                         kMinEpipolarErrSensitivity_ : normalization_factor;
...
//   const float kCovarianceNormalizationCoeff = projection_mat_cam0_(0,0);
  ...
  cv::Mat tf_prev_to_curr = CalculateRelativeTransform(curr_frame.mTwc_gt,
                                                       ref_frame_Twc_gt);
  Vector3d trans_prev_to_curr(double(tf_prev_to_curr.at<float>(0,3)),
                            double(tf_prev_to_curr.at<float>(1,3)),
                            double(tf_prev_to_curr.at<float>(2,3)));
...
//                                   trans_prev_to_curr.cast<float>());
  ...
  mean << euler_angles.cast<float>(),
          trans_prev_to_curr.cast<float>();
 
  
  // Covariance matrix:
  Eigen::Matrix<float, 6, 6> covariance;
  covariance << kAngualrVariance, 0 , 0, 0, 0, 0, 
...
//   weights[0] = lambda_coeff / (n + lambda_coeff);
  ...
  for (size_t i = 0; i < static_cast<size_t>(2 * n); i++) {
    ...
    if (i < n) {
//       sigma_pt = mean.array() 
//                   + ((n + lambda_coeff) * covariance.col(i)).array().sqrt();
      sigma_pt = mean.array() 
                  + (covariance.col(i)).array().sqrt();
    } else {
//       sigma_pt = mean.array() - 
//                  ((n + lambda_coeff) * covariance.col(i - n)).array().sqrt();
      sigma_pt = mean.array() - 
                 (covariance.col(i - n)).array().sqrt();
    }
...
//     weights[i + 1] = 1.0 / (2 * (n + lambda_coeff));
    ...
    Vector3f trans_prev_to_curr_particle(sigma_pt(3), 
                                         sigma_pt(4), 
                                         sigma_pt(5)); 
    ...
    Vector2f err_vec_particle = CalculateEpipolarErrorVec(
                                    R_prev_to_curr_particle.cast<double>(),
...
//   }
  ...
  *sigma_pts_err = particles_err_vec;
  ...
  // The standard deviation
  float abs_err_std = sqrt(1.0 / static_cast<float>(particles_err_vec.size()) 
                          * squared_sum); 
  ...
  float abs_err_std_m = sqrt(1.0 / static_cast<float>(particles_err_vec.size()) 
                          * squared_sum_m); 
  ...
  //   cout << "float cov det: " << predicted_covariance.determinant() << endl;
...
void FeatureEvaluator::GetEpipolarErrorJacobians(
  ...
  cam_mat <<        cam_frame.fx, 0.0,          cam_frame.cx,
                    0.0,          cam_frame.fy, cam_frame.cy,
                    0.0,          0.0,          1.0;
  
  // The jacobians of the epipolar line
  Eigen::Matrix3d Jl_w, Jl_t;
  GetEpipolarLineJacobians(cam_mat,
                            R_prev_to_curr,
                            trans_prev_to_curr,
                            x_ref_n,
                            &Jl_w,
  ...
  Eigen::Matrix3d F = GetFundamentalMatrix(cam_mat,
                                          R_prev_to_curr,
                                          trans_prev_to_curr);
...
void FeatureEvaluator::GetEpipolarLineJacobians(const Eigen::Matrix3d& cam_mat,
                                const Eigen::Matrix3d& R_prev_to_curr,
                                const Eigen::Vector3d& trans_prev_to_curr,
                                const Eigen::Vector3d& x_ref,
                                Eigen::Matrix3d* J_w,
...
//   it_curr = rel_cam_poses_unc_map_->find(curr_frame_name);
  ...
  rel_pos_cov->bottomRightCorner(3,3) *= (max_trans_unc / scale95) * 
                                         (max_trans_unc / scale95);
  rel_pos_cov->topLeftCorner(3,3) *= (max_rot_unc /(scale95 * sqrt(3))) * 
                                     (max_rot_unc /(scale95 * sqrt(3)) );
...
void FeatureEvaluator::CalcRelativePoseError(cv::Mat& pose_est_0, 
  ...
  Eigen::Map<Eigen::Matrix<float, 4, 4,Eigen::RowMajor>> 
                                      T_est0(pose_est_0.ptr<float>(), 
                                            pose_est_0.rows, 
                                            pose_est_0.cols);
  Eigen::Map<Eigen::Matrix<float, 4, 4,Eigen::RowMajor>> 
                                      T_est1(pose_est_1.ptr<float>(), 
                                            pose_est_1.rows, 
                                            pose_est_1.cols);
  Eigen::Map<Eigen::Matrix<float, 4, 4,Eigen::RowMajor>> 
                                      T_gt0(pose_gt_0.ptr<float>(), 
                                            pose_gt_0.rows, 
                                            pose_gt_0.cols);
  Eigen::Map<Eigen::Matrix<float, 4, 4,Eigen::RowMajor>> 
                                      T_gt1(pose_gt_1.ptr<float>(), 
                                            pose_gt_1.rows, 
                                            pose_gt_1.cols);
  ...
  t_est_curr_H = R_est_curr.transpose() * t_est_H - R_est_curr.transpose() * 
                                          t_est_curr;
...
void FeatureEvaluator::EvaluateAgainstPrevFrame(
  ...
  for(size_t i = 0; i < curr_frame.mvKeysUn.size(); i++) {
    ...
    if(curr_map_pt) {
      ...
      if (prev_frame.GetCorrespondingKeyPt(curr_map_pt, &keypt_prev)){
        ...
        cv::Point2f reproj_pt;
        float reproj_err = CalculateReprojectionError(*pt_ref_keyframe,
                                                      curr_frame,
                                                      *curr_map_pt,
                                                      keypt_curr,
                                                      &reproj_pt);
...
void FeatureEvaluator::EvaluateAgainstPrevFrameEpipolar(
  ...
  for(size_t i = 0; i < curr_frame.mvKeysUn.size(); i++) {
    ...
    if(curr_map_pt) {
      ...
      bool prev_keypt_available = prev_frame.GetCorrespondingKeyPt(curr_map_pt, 
                                          &keypt_prev, 
                                          &keypt_prev_idx);
    ...
    if(curr_map_pt) {
      ...
      if (prev_keypt_available){
        ...
        if (kUseAnalyticalUncertaintyPropagation_) {
          ...
          epipolar_err_covariance << epipolar_err_var, 0,
                                     0      , epipolar_err_var;
        ...
        if (isnan(err) || err < 0) {
          LOG(WARNING) << "Invalid normalized error value " << err << 
                      ".Error covariance should have a positive determinant."
                      << "The covariance matrix was " << 
                      epipolar_err_covariance << endl
                      << "determinent: " << 
                          epipolar_err_covariance.determinant();
          continue;
        }
        ...
        keypts2_select_ref_frame_offset_.push_back(curr_frame.mnId
                          - curr_map_pt->GetReferenceKeyFrame()->mnFrameId);
        keypts2_matched_w_prev_.push_back(keypt_curr);
        keypts1_matched_w_curr_.push_back(keypt_prev);
        int keypt_idx = (int)(keypts2_matched_w_prev_.size()) - 1;
        matches1to2_.push_back(DMatch(keypt_idx, keypt_idx, 0.0));
        ...
        if(gt_depth_available) {
          if (uncertain_gt_depth) {
            ...
            if (kOverwriteWithReprojErr) {
              ...
              if (false) {
                float keypt_sigma = 1 / 
                                curr_frame.mvInvLevelSigma2[keypt_curr.octave];
                float scale95 = sqrt(5.991);
                float normalization_factor =  scale95 * 
                                          static_cast<float>(sqrt(keypt_sigma));
...
Eigen::Vector2f FeatureEvaluator::CalculateEpipolarErrorVec(
  ...
  CalculateNormalized3DPoint(cv::Point2f(keypoint1.pt.x, keypoint1.pt.y),
                             0,
                             &norm_3d_points1);
  CalculateNormalized3DPoint(cv::Point2f(keypoint2.pt.x, 
                                         keypoint2.pt.y),
                             0,
                             &norm_3d_points2);
  ...
  Vector3f c1_in2_3d = R_prev_to_curr.cast <float>() * c1_in1 + 
                      trans_prev_to_curr.cast <float>();
...
double FeatureEvaluator::CalculateEpipolarError(
  ...
  CalculateNormalized3DPoint(cv::Point2f(keypoint1.pt.x, keypoint1.pt.y),
                             0,
                             &norm_3d_points1);
  CalculateNormalized3DPoint(cv::Point2f(keypoint2.pt.x, keypoint2.pt.y),
                             0,
                             &norm_3d_points2);
  
  double err = fabs(norm_3d_points2.transpose() 
                    * essential_mat 
                    * norm_3d_points1);
...
double FeatureEvaluator::CalculateEpipolarError(const cv::Mat& tf_prev_to_curr,
                                      const ORB_SLAM2::Frame& curr_frame,
                                      const cv::KeyPoint& keypoint1,
                                      const cv::KeyPoint& keypoint2,
                                      Eigen::Vector2f* epipolar_line_dir,
...
Eigen::ArrayXXd FeatureEvaluator::ClampArray(const Eigen::ArrayXXd& input_array,
  ...
  for (int i = 0; i < input_array.size(); i++) {
    clamped_array(i) = (input_array(i) > max_value)? max_value : input_array(i);
  }
...
cv::Mat FeatureEvaluator::CalculateRelativeTransform(
  ...
  Eigen::Matrix3d Sigma_w;
  Sigma_w = J_w_w1 * src_frame_pose_cov.topLeftCorner(3,3) * J_w_w1.transpose()
        + J_w_w2 * dest_frame_pose_cov.topLeftCorner(3,3) * J_w_w2.transpose();
  
  // Covariance of the translational part of tf_dest_src
  Eigen::Matrix3d Sigma_t;
  Sigma_t = J_t_t1 * src_frame_pose_cov.bottomRightCorner(3,3) * 
            J_t_t1.transpose()
        + J_t_t2 * dest_frame_pose_cov.bottomRightCorner(3,3) * 
            J_t_t2.transpose()
        + J_t_w2 * dest_frame_pose_cov.topLeftCorner(3,3) * 
            J_t_w2.transpose();
...
Eigen::Vector2f FeatureEvaluator::ProjectToCam(
  ...
  Vector3f projection = (cam_mat * point_3d_in_cam_ref)/point_3d_in_cam_ref(2);
