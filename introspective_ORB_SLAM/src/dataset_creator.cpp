...
DatasetCreator::DatasetCreator(const std::string& dataset_path):
...
void DatasetCreator::SaveBadRegionHeatmap(const std::string& img_name,
                                        const cv::Mat& bad_region_heatmap) {
  string img_dir = dataset_path_ + "/bad_region_heatmap/";
  if(!CreateDirectory(img_dir)) {
    LOG(FATAL) << "Could not create the direcotory for saving the heatmaps";
  }
...
void DatasetCreator::SaveBadRegionHeatmapMask(const std::string& img_name,
