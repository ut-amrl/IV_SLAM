...
DEFINE_int32(tracking_ba_rate, 1, 
...
DEFINE_double(ivslam_ref_pose_ang_var_inv, 2.0e+3,
...
DEFINE_double(ivslam_ref_pose_trans_var_inv, 2.0e+2,
              "Inverse of the variance of the reference camera "
...
&strSettingPath, const int sensor, const bool bSingleThreaded, 
const bool bSilent, const bool bGuidedBA):
    mState(NO_IMAGES_YET), mSensor(sensor), mbSingleThreaded(bSingleThreaded),
    ...
    mnLastRelocFrameId(0), mbSilent(bSilent), mbGuidedBA(bGuidedBA)
{
    ...
    if (!introspective_ext.empty()) {
      enableInrospectiveFeatExtraction = static_cast<bool>(int(
                  fSettings["ORBextractor.enableIntrospection"]));
    }
    ...
    if(sensor==System::STEREO || sensor==System::RGBD)
    {
        ...
        if (!bSilent) {
        cout << endl << "Depth Threshold (Close/Far Points): " 
             << mThDepth << endl;
        }
    }
    ...
    if (!search_window.empty()) {
      ...
      if (!bSilent) {
        cout << "ORBMatcher.SearchWindowMultiplier: "
            << mMatcherSearchWindowMultiplier << endl << endl;
      }
    }
    
    // IVSLAM training parameters
    cv::FileNode unsupervised_learning = 
                  fSettings["IVSLAM.unsupervisedLearning"];
    if (!unsupervised_learning.empty()) {
      mbUnsupervisedLearning = static_cast<bool>(int(
                  fSettings["IVSLAM.unsupervisedLearning"]));
    } else {
...
cv::Mat Tracking::GrabImageStereo(const cv::Mat &imRectLeft, 
    ...
    mCurrentFrame = Frame(mImGray,imGrayRight,timestamp,mpORBextractorLeft,
                          ...
                          mbf,mThDepth, gtDepthAvailable, depthmap,
...
//           mFeatureEvaluator->GenerateImageQualityHeatmap();
            ...
            // estimations
            mFeatureEvaluator->GenerateUnsupImageQualityHeatmapGP(
                                                          mCurrentFrame);
...
//               mDatasetCreatorFull = 
//                   new DatasetCreator(mvOutputIntrospectionDatasetFullPath);
//             }
        ...
        if(mState==LOST)
        {
          if (mbIntrospectionOn || true) {
              if(mpMap->KeyFramesInMap()<=3) {
                  ...
                  mFailureCount++;
                  LOG(WARNING) << "Failure #" << mFailureCount 
                               << ". Resetting...";
...
void Tracking::CreateInitialMapMonocular()
{
    ...
    for(size_t i=0; i<mvIniMatches.size();i++)
    {
        ...
        if(FLAGS_ivslam_propagate_keyptqual) {
          float qual_score = std::min(mInitialFrame.mvKeyQualScore[i],
...
bool Tracking::TrackReferenceKeyFrame(float nn_ratio_mult, 
    ...
    if (use_BoW) {
      ...
      nmatches = matcher.SearchByBoW(mpReferenceKF,
                                       mCurrentFrame,
                                       vpMapPointMatches);
    ...
    if (kEnableKeyPointEval) {
      for(size_t i = 0; i < mCurrentFrame.mvKeysUn.size(); i++) {
        ...
        if(curr_map_pt) {
          ...
          bool prev_keypt_available = 
                      mLastFrame.GetCorrespondingKeyPt(curr_map_pt, 
                                                      &keypt_prev, 
                                                      &keypt_prev_idx);
...
//             double err_log = log(err);
  ...
  //           cout << "reproj err vec: " << reproj_err.transpose() << endl;
  //           cout << "scaled err: " << scaled_err << endl; 
...
//       cout << "curr frame ID: " << curr_frame.mnId << endl;
          int pt_idx_in_frame = 
                    curr_map_pt->GetIndexInKeyFrame(pt_ref_keyframe);
...
//     std::cout << "TrackLocal map inliers: " << mnMatchesInliers << std::endl;
...
void Tracking::VisualizeKeyPtsQuality() {
  ...
  const string src_dir = "/media/ssd2/datasets/Jackal_Visual_Odom/"
...
//   KeyFrame* key_frame = mpLastKeyFrame;
  ...
  cv::Mat img = cv::imread(src_dir + "/" + img_name,
...
//   cout << "curr_frame/found_ref:  " << curr_id << " / " << ref_kf->mnFrameId 
//        << endl;
  ...
  mFeatureEvaluator->CalcRelativePoseError(pose_est_0, 
                                           pose_est_1,
                                           pose_gt_0,
                                           pose_gt_1,
                                           &aa_rot_err,
...
//        << aa_rot_err.angle() << endl;
  ...
  cam_pose_cov_inv << FLAGS_ivslam_ref_pose_ang_var_inv, 0, 0, 0, 0, 0,
                      0, FLAGS_ivslam_ref_pose_ang_var_inv, 0, 0, 0, 0,
                      0, 0, FLAGS_ivslam_ref_pose_ang_var_inv, 0, 0, 0,
                      0, 0, 0, FLAGS_ivslam_ref_pose_trans_var_inv, 0, 0,
                      0, 0, 0, 0, FLAGS_ivslam_ref_pose_trans_var_inv, 0,
                      0, 0, 0, 0, 0, FLAGS_ivslam_ref_pose_trans_var_inv;
...
//   cout << "chi2: " << chi2 << " / " << chi2_thresh << ". ";
  ...
  if(kAssertMinVelocity) {
    ...
    if(kUseTimeStamps) {
      ...
      del_t = (1.0 / mMaxFrames) * (mCurrentFrame.mnId 
                                   - mpLastKeyFrame->mnFrameId);
    }
    
    Eigen::Map<Eigen::Matrix<float, 4, 4,Eigen::RowMajor>> 
                                      T_rel(rel_tf.ptr<float>(), 
                                            rel_tf.rows, 
                                            rel_tf.cols);
...
//       cout << "Velocity does not reach the minimum for training data gen" 
//            << endl;
      return false;
    }
  }
...
void Tracking::SearchLocalPoints()
{
    ...
    if(nToMatch>0)
    {
        ...
        matcher.SearchByProjection(mCurrentFrame,
                                   mvpLocalMapPoints, 
                                   mMatcherSearchWindowMultiplier * th);
    }
}
...
bool Tracking::Relocalization()
{
    ...
    if (mpKeyFrameDB) {
    vpCandidateKFs = mpKeyFrameDB->DetectRelocalizationCandidates(
                                   &mCurrentFrame);
    }
...
void Tracking::SaveTrackingResults(bool saving_on_failure) {
  ...
  if (mbSaveVisualizationsToFile) {
    ...
    path_to_traj = dir + "KeyFrameTrajectory_TUM_" 
                        + suffix + ".txt";
    path_to_failure_log = mvSaveVisualizationPath + 
                         "/failure_log.txt";

    string dir_kitti = mvSaveVisualizationPath + "/trajectory_kitti/";
    path_to_traj_kitti = dir_kitti + "Trajectory_KITTI_" 
                        + suffix + ".txt";
  ...
  if (saving_on_failure) {
    ...
    failure_log << std::setprecision(20) 
                  << mlFrameTimes.back() << std::endl;
